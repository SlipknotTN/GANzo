...
def generator(inputs, out_channel_dim=3, alpha=0.2, keepProb=0.7, isTraining=True):
    """
    Create the generator network
    :param inputs: Tuple of noise and labels
    ...
    # Conditioning part
    x1 = tfgan.features.condition_tensor_from_onehot(x1, oneHotLabel)
    ...
    # Single image of batch visualization
    tf.summary.image(
         "generated",
         out,
         max_outputs=1,
...
def discriminator(x, conditioning, alpha=0.2, keepProb=0.7, isTraining=True):

    noise, oneHotLabel = conditioning
    ...
    x = tfgan.features.condition_tensor_from_onehot(x, oneHotLabel)
