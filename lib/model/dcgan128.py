...
def generator(z, out_channel_dim=3, alpha=0.2, keepProb=0.7, isTraining=True):
    ...
    # 6th block -> 128x128xCH (no batch normalization and use tanh activation)
    logits = tf.layers.conv2d_transpose(x5, filters=out_channel_dim, kernel_size=(5, 5), strides=2,
                                        padding='same', activation=None,
                                        kernel_initializer=tf.contrib.layers.xavier_initializer())
